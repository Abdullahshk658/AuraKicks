...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-5 text-left text-sm font-semibold text-white transition-all hover:text-brand-100 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
